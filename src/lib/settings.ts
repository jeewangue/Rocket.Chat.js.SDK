...
export let host = process.env.ROCKETCHAT_URL || "localhost:3333";
...
export let rooms = process.env.ROCKETCHAT_ROOM
  ...
  : [];
export let allPublic =
  (process.env.LISTEN_ON_ALL_PUBLIC || "false").toLowerCase() === "true";
export let dm = (process.env.RESPOND_TO_DM || "false").toLowerCase() === "true";
export let livechat =
  (process.env.RESPOND_TO_LIVECHAT || "false").toLowerCase() === "true";
export let edited =
  (process.env.RESPOND_TO_EDITED || "false").toLowerCase() === "true";
...
export let roomCacheMaxAge =
  1000 * parseInt(process.env.ROOM_CACHE_MAX_AGE || "300", 10);
export let dmCacheMaxSize = parseInt(
  process.env.DM_ROOM_CACHE_SIZE || "10",
  10
);
export let dmCacheMaxAge =
  1000 * parseInt(process.env.DM_ROOM_CACHE_MAX_AGE || "100", 10);
